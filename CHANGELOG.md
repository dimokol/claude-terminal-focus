...
- **Windows: foreground-raise plumbing for the banner click.** With the UriHandler architecture, VS Code's URI activation arrives in the (background) extension host, which has no foreground rights — so a click left VS Code behind whatever app was in front. Groundwork laid here: extracted the `bin/win-click-handler.js` focus-steal logic into a shared `lib/win-focus.js` that resolves the exact renderer Code.exe HWND from the marker's PID chain (`resolveCodeInstancePids` — all Code.exe ancestors, since the shortest-walk one is the windowless ptyHost) and raises it via a non-detached PowerShell helper (`AttachThreadInput` + Alt-key tap + `BringWindowToTop` + `SetForegroundWindow`, retried across the post-click foreground-settling window). **This alone is not sufficient** — see the `ForegroundLockTimeout` entry below for why the raise only takes effect once that OS setting is `0` (the `forceForeground` opt-in). `detached: true` was found to BREAK the raise (a new process group loses the input-inheritance) and was removed.
