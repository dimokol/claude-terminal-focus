...
### Changed
- **Hooks now install with `async: true`, so Claude no longer blocks on them (issue #2).** Claude Code fires the hook and continues immediately instead of waiting for it to exit, removing the ~1.2s "running Stop hook" pause that remained after the detached-sound fix (`3145f00`). That pause was the deliberate `await sleep(handshakeMs)` handshake the hook uses to give the focused VS Code extension a chance to claim the event for an in-window toast. With `async`, the hook still runs that full handshake → claim → notify sequence in the background on the same timeline (only Claude's wait is removed), so the in-window-vs-OS-banner decision, the stage-dedup state machine, and the claim race are all unchanged. The flag is set on the three notification hooks (Stop / Notification / PermissionRequest) but **not** on UserPromptSubmit, which advances the stageId synchronously with no handshake; keeping it synchronous preserves its happens-before ordering relative to the next Stop. Older Claude Code builds that don't recognize the field ignore it and behave exactly as before. Trade-off: the banner now lands ~1s after the turn visibly completes rather than together with it (usually reads as snappier). A present, fast-typing user can in principle see a Stop banner for a turn they've already moved past, but a present user gets the quiet in-window toast and doesn't depend on the banner; the away user who needs it isn't typing fast follow-ups.

### Fixed
- **Duplicate and late-repeat notifications: now exactly one notification per attention point.** Users reported a session firing a notification twice (a completion, then a second alert seconds later) and sometimes a third 1-2 minutes later, for the same "Claude needs you" moment. Root-caused 2026-05-29 by instrumenting the full hook stream: every attention point is announced by a PRIMARY event: `Stop` (completion) or `PermissionRequest`/`PreToolUse` (a new question / tool request; each `AskUserQuestion` fires its own `PermissionRequest`), while a bare `Notification` is always a trailer or "still waiting" re-fire of it ("Claude is waiting for your input", "Claude needs your permission"). The old time-based escape valve let any event escape after 3s, so a trailing/late `Notification` re-fired as a second notification. Fix in `lib/stage-dedup.js`: a `Notification` never escapes the burst window (always suppressed as a trailer); only a primary event after the window is treated as a genuinely new attention point. This guarantees **at least one** notification for every question and completion (back-to-back `AskUserQuestion`s each notify, since answering one does not emit `UserPromptSubmit`, confirmed empirically, but each fires its own `PermissionRequest`) and **at most one** (all trailers and re-fires collapse). Unit tests rewritten against the real captured event sequences. Independent of the `async` change above.
...
- **Windows: banner click reliably switches to the correct terminal in the correct window.** The toast `launch=` routes to the extension's `registerUriHandler`, which decodes the marker and focuses the matching terminal via the tiered matcher. (Bringing the VS Code *window* itself to the foreground on a banner click is a separate, unsolved Windows limitation: `ShellExperienceHost` owns the foreground at click time and the OS blocks any background raise over it; the taskbar flashes instead. A full investigation, including why it can't currently be done and how to approach it next, is in `docs/windows-banner-focus-handoff.md`.)
...
- **Windows: clicking a notification opened a new Stable VS Code window instead of your VS Code Insiders instance (issue #4).** The toast `launch=` URI was a hardcoded `vscode://...`, and Windows routes `vscode://` to whichever build owns the scheme (normally Stable), so Insiders users (and forks) never reached the window the notification came from. The hook now detects the VS Code build hosting the Claude session from the process-ancestor chain (`Code.exe`, `Code - Insiders.exe`, `VSCodium.exe`, `Cursor.exe`, `Windsurf.exe`) and emits the matching product scheme (`vscode-insiders://`, `vscodium://`, `cursor://`, `windsurf://`), falling back to `vscode://` when no VS Code ancestor is found. Each build's `registerUriHandler` answers its own product scheme, so the click reaches the correct instance, which the existing PID-chain terminal matcher then switches to the right tab. New `lib/code-build.js` (build classification plus scheme/CLI lookup) with unit tests. Thanks [@mlidbom](https://github.com/mlidbom) for filing [#4](https://github.com/dimokol/claude-notifications/issues/4) and suggesting the per-build scheme.
...
- **Auto-migration on upgrade.** First 3.5.0 activation detects pre-3.5.0 direct-to-extension hook entries (by matching `dimokol.claude-notifications` in the command string) and rewrites them to point at the wrapper. Stale hook entries from old extension dirs no longer accumulate across upgrades. This is the fix for the `MODULE_NOT_FOUND`-on-update breakage; thanks [@iodar](https://github.com/iodar) for filing [#1](https://github.com/dimokol/claude-notifications/issues/1) with a precise root-cause and the exact stable-wrapper suggestion.
