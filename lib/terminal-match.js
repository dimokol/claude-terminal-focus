...
function matchTerminal(terminals, signal) {
  ...
  // --- Tier 2.5: aiTitle exact substring match ---
  // Claude Code writes the session's auto-generated task title to the
  // terminal name via ANSI title escapes — e.g.
  //   "✳ Address claude notifications open issues"
  // Each running Claude session has a UNIQUE title, so this is the most
  // discriminating heuristic when multiple Claude sessions share a
  // workspace (where tier=cwd and the generic ✳ claude-marker tier both
  // go ambiguous and fall through to null).
  // Min length 4 to avoid trivial matches (e.g. short generic titles
  // colliding with terminal name suffixes like " (1)").
