...
function checkHookStatus(expectedHookPath, settingsPath = CLAUDE_SETTINGS_PATH) {
  ...
  try {
    ...
    if (hasAllPrimary && hasUserPrompt) {
      ...
      // Path is current but the entry shape may be stale. The notification
      // hooks must carry async:true (added after the 3.5.x wrapper era); an
      // install made before that change has correct paths but no async, so
      // it would otherwise read as 'installed' and never get rewritten — the
      // async feature would never reach it. Flag it so activation reinstalls.
      if (!notificationHooksAreAsync(hooks)) {
