...
A big maintenance release rolling up everything since the last Marketplace publish (v3.3.1).

- **Windows notifications fire reliably again.** Previous 3.5.x builds silently dropped the OS toast in some setups because the PowerShell child that registers the toast was getting killed inside Claude Code's hook job before reaching `ToastNotificationManager.Show()`. Restored the `cmd /c start "" /B` wrapper so PowerShell detaches into a fresh process group that survives the job teardown.
- **No more `MODULE_NOT_FOUND` after VS Code uninstalls or updates the extension.** Hooks now point at a stable wrapper under `~/.claude/claude-notifications/` instead of the versioned extension dir. When the extension is gone, the wrapper detects this on its next fire and self-cleans across every Claude profile, the Windows registry handler, and the per-workspace state. Thanks **[@iodar](https://github.com/iodar)** for filing [#1](https://github.com/dimokol/claude-notifications/issues/1) with a precise root-cause and the exact wrapper-script suggestion.
...
- **Windows duplicate hook entries fixed.** A path-quoting bug caused every VS Code restart to append a duplicate entry to `~/.claude/settings.json` on Windows (a tester reached 12 entries per event). Plain double-quoting now, with an idempotency regression test.
- **`AskUserQuestion` multi-choice prompts fire reliably.** Escape valve in stage dedup catches follow-up questions that previously got silenced as same-stage bursts. Fast banner-click no longer fires a duplicate sound (the platform PR+Notification burst is now correctly collapsed even when the user clicks between the two events).
- **Banner clicks land on the right VS Code window with multi-instance setups.** On Windows, clicks now use Win32 `SetForegroundWindow` on the originating instance's renderer process — no more landing in whichever window happened to be most recently focused.
- **Right terminal focused on Git Bash + Windows.** Tiered terminal matching (PID → shell-integration cwd → Claude title markers → single non-default-shell-named) replaces the previous PID-only match that silently broke under MSYS2/winpty/ConPTY indirection.
...
- **New: `claudeNotifications.toastWhenFocused`** (default `false`). Opt-in visual toast when you're already on the Claude terminal — useful for multi-monitor or small-terminal-panel setups where the audio cue alone can be missed.

Massive thanks to **[@AdaWanheda](https://github.com/AdaWanheda)** for thorough Windows testing across PowerShell, Git Bash, and multi-monitor setups. Several of the bugs above were surfaced by her test cycles.
